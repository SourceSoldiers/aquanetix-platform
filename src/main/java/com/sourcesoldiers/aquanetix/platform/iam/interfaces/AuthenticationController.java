package com.sourcesoldiers.aquanetix.platform.iam.interfaces;
...
public class AuthenticationController {
    ...
    @Operation(
            ...
            operationId = "SignIn",
            description = "Authenticates a user with provided email and password and returns JWT token for subsequent requests."
    )
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
     ...
     * @param signUpResource the sign-up request body with email, password, and plan.
     * @return the authenticated user resource with JWT token.
     */

    @PostMapping("/sign-up")
    @Operation(
            summary = "Sign up a new user with a chosen plan",
            operationId = "SignUp",
            description = "Creates a new user account with provided credentials and chosen subscription plan."
    )
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
